...
## 近期阶段状态
- 当前稳定提交：`97e6cc0 docs: record failed crop-first baseline experiment`
...
3. 回到 Phase 4M route selection；subject/background relationship 仍重要，但 pure safe crop 太保守，region-bg-quiet v1 又会误伤 subject-dominant / frame-touching subject。下一步若继续 first-draft desirability，应优先考虑 color quantization order redesign；若未来重访 region-aware，必须先有 subject-protection design，而不是继续加强 background thresholds。
