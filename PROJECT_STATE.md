...
## 近期阶段状态
- 当前稳定提交：`db6bde1 docs: record failed region-aware background experiment`
...
3. 回到 Phase 4M route selection；已测试路线 simplification-v3、crop-first v1、region-bg-quiet v1、quantization-order v1 均失败 / non-integrated。下一步应先做 higher-level route review，而不是立刻继续参数实验；若未来重访 region-aware，必须先有 subject-protection design，而不是继续加强 background thresholds。
